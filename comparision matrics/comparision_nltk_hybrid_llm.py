...
HYBRID_LLM_FILES = {
    "hybrid-llm": r"dblp_keywords_hybrid_parallel_retry.jsonl",
}
...
llm_data = {}
for model_name, file_path in HYBRID_LLM_FILES.items():
    ...
    if os.path.exists(full_path):
        ...
        if data:
            ...
            print(f"  Sample cleaned for {model_name}: {sample_cleaned[:100]}...")  # Truncate for print
    else:
...
debug_file = r"debug_hybrid_samples.json"
...
if len(common_papers) == 0:
    ...
    for paper_id in common_papers:
        ...
        for model_name, llm_dict in llm_data.items():
            ...
            results.append({
    'paper_id': paper_id,
    'model_display': paper_llm.get('model_display', model_name),
    'full_model_name': paper_llm.get('full_model_name', model_name),
    ...
    summary = df.groupby('full_model_name').agg({
    ...
    best_model = df.groupby('full_model_name')['avg_jaccard'].mean().idxmax()
    ...
    output_csv = r"metrics_nltk_hybrid_comparison.csv"
